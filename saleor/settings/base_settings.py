...
def get_privates_from_env():
    file_path = os.environ.get("START_FILE", None)
    if not file_path:
        raise ImproperlyConfigured(
            "Need to set the START_FILE env variable"
            )
    else:
        pf = os.path.join(PROJECT_ROOT, file_path)
        print(pf)
        with open(pf, 'r') as open_file:
            try:
                privates = json.load(open_file)
                return privates
...
PRIVATES = get_privates_from_env()


def get_sec(setting, sec=PRIVATES):
    """Get the sec variable or return exception"""
    try:
        return PRIVATES[setting]
    except KeyError:
        error_msg = f"Set the {setting} environment variable please!"
